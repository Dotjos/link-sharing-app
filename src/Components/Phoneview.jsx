  ...
  return ( 
       <div className="hidden w-2/5 lg:flex items-center justify-center bg-white rounded-lg p-4">
        ...
        <div className="bg-white rounded-lg  bottom-4 w-5/12 h-2/3 overflow-y-auto absolute">
          {addedLinks.map(addeDetails=><PhoneLink key={addeDetails.linkId} link={addeDetails.details?.link} platform={addeDetails.details?.platform} icon={addeDetails.details?.img}  background={addeDetails.details?.color || "bg-LavenderMist"}/>)}
         </div>
        </div>
      </div>
  );

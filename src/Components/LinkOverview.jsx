...
function LinkOverview (){
  ...
  const refinedUserLinkDetails = userLinkDetails?.filter((item) => {
    return item.details && typeof item.details === 'object' && Object.keys(item.details).length && !item.details.error && item.details.linkInput!==""
  });
...
return(
  ...
  <div className="bg-white h-full w-full ml-auto mr-auto md:w-1/3 p-7 rounded-lg">
    ...
    >
      {(!imageURL||urlStatus==="error")&& <span className="font-instrumentSansSemiBold">{firstName.charAt(0)}{lastName.charAt(0)}</span> }
           </div>

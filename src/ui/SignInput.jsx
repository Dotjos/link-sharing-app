function SignInput ({icon,placeholder,type,label,name,onChange,value,error,errMessage,disabled}){
    ...
    <div className="my-4 focus:shadow-xl ">
     <label htmlFor={name}>
      ...
      <span className="text-right lg:w-1/4 md:hidden text-LightRed text-xs">{error&&errMessage}</span>
            ...
            {error&&<span className="hidden md:inline-block text-right lg:w-1/4 text-LightRed text-xs">{errMessage}</span>}
            </div>
       </label>
    </div>
  );
